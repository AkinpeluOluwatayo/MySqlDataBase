...
export default function EditNumber() {
    ...
    const handleUpdate = async (e) => {
        ...
        const loadingToast = toast.loading("Syncing secure line...", {
            style: { background: '#04120a', color: '#27AE60', border: '1px solid #27AE60' }
        });

        updateProfile.mutate({ phoneNumber: phoneNumber.trim() }, {
            onSuccess: () => {
                toast.success("Phone Number Updated", { id: loadingToast });
                router.back();
            },
            onError: (err) => {
                const msg = err?.response?.data?.message || "Could not update number. Tactical link interrupted.";
                setError(msg);
                ...
                >
                    ...
                    <div className="mb-10">
                        <h1 className="text-4xl font-black tracking-tighter text-white">
                            Edit <span className="text-[#27AE60]">Phone Number</span>
                        </h1>
                        <p className="mt-2 text-emerald-50/40 font-medium">
                            Update your secure communication line.
                        </p>
                    </div>
